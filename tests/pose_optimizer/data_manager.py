...
class DataManager:
    def __init__(self, feature_points_position,
                       cameras_position,
                       cameras_rotation,
                       log_file_name):
        ...
        self.related_feature_points_position = self.get_related_feature_points_position()
    ...
    def get_keyframes_bundle(self, noise_scale):
        ...
        for fp, t, R in zip(self.related_feature_points_position, self.cameras_true_position, self.cameras_true_rotation):
            noise = np.random.normal(0, noise_scale, 3)
            bundle = [ [ i, ( point + noise ) / np.linalg.norm(point) ] for i, point in enumerate(fp) ]
    ...
    def get_noised_keyframe_rotaion(self, noise_scale):
        noise = [ np.random.normal(0, noise_scale, 3) for p in self.cameras_true_rotation]
        noise = [ scipy_R.from_euler('xyz', n).as_matrix() for n in noise ]
        return [ noise[i] @ rotation for i, rotation in enumerate(self.cameras_true_rotation) ]
    ...
    def get_related_feature_points_position(self):
        return [ self.translate_points(self.feature_points_true_position, t, R) for t, R in zip(self.cameras_true_position, self.cameras_true_rotation) ]
    ...
    def _setup_logger(self, file_name):
        ...
        fp_trajectory_log = StateLogger(log_dir + 'fp_trajectory.csv',
                                        tuple(labels))
        fp_true_position_log = StateLogger(log_dir + 'fp_true_position.csv',
                                           tuple(labels))
        ...
        kf_trajectory_log = StateLogger(log_dir + 'kf_trajectory.csv',
                                        tuple(labels))
        kf_true_position_log = StateLogger(log_dir + 'kf_true_position.csv',
                                           tuple(labels))
