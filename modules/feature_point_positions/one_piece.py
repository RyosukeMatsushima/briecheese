...
class OnePiece:
    def __init__(self):
        ...
        self.keyframes = {}  # {keyframe_id: ObservedData}
    ...
    def add_keyframe(
        ...
        for related_feature_point_direction in related_feature_point_directions:
            self.feature_point_ids[related_feature_point_direction[0]].append(
                [self.keyframe_number, related_feature_point_direction[1]]
            )

        for new_feature_point_direction in new_feature_point_directions:
            self.feature_point_ids.update(
                {
                    new_feature_point_direction[0]: [
                        [self.keyframe_number, new_feature_point_direction[1]]
                    ]
                }
            )

        self.keyframes.update(
            {
                self.keyframe_number: ObservedData(
