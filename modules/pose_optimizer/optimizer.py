...
class Optimizer:
    ...
    def optimize(self, max_trial, optimize_feature_point=True, callback=None):
        ...
        while not is_enough:
            ...
            evaluate_value = (
                np.linalg.norm(np.sum(feature_points_force, axis=0))
                / np.shape(self.feature_points_position)[0]
            )
            is_enough = (
    ...
    def calculate(self, keyframe):
        ...
        feature_points_force = np.zeros(np.shape(self.feature_points_position))
        keyframe_force_constant = self.position_constant / len(
            keyframe.feature_points_bundle
        )
        keyframe_moment_constant = self.rotation_constant / len(
            keyframe.feature_points_bundle
        )
        ...
        for feature_point_bundle in keyframe.feature_points_bundle:
            ...
            keyframe_moment = (
                cv.Rodrigues(-moment * keyframe_moment_constant)[0] @ keyframe_moment
            )
            feature_points_force[feature_point_id] += force * self.position_constant

        if keyframe.position_bundle.size != 0:
            keyframe_force -= (
                keyframe.position - keyframe.position_bundle
            ) * self.position_bundle_constant

        if keyframe.rotation_bundle.size != 0:
            m = (
                cv.Rodrigues(
                    keyframe.rotation @ np.linalg.inv(keyframe.rotation_bundle)
                )[0].T[0]
                * self.rotation_bundle_constant
            )
